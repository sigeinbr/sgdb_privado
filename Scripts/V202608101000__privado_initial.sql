 ...
 *
 * `adm.usuarios` é escopado por conta (`conta_id`, com `login`/`email`/`cpf` únicos por
 * conta, não globalmente) — um usuário pertence a exatamente uma conta. Isso é uma
 * mudança deliberada em relação ao Sigein original (onde `adm.usuarios` é global e um
 * login pode acessar várias UGs): aqui não há cenário de um usuário administrando várias
 * contas, então a tabela nasce já dentro da conta.
 */
 ...
 * TABELA: adm.contas
 *
 * Tenant de topo do sistema (substitui adm.ugs do Sigein). Uma "conta" é uma empresa
 * privada cadastrada via autoatendimento. `id` é identity (sequence real), não smallint
 * manual, pois o crescimento via autoatendimento não é previsível como o número de UGs.
 */
 ...
 * TABELA: adm.usuarios
 *
 * Escopada por conta — um usuário pertence a exatamente uma conta (não é mais global).
 * `login`, `email` e `cpf` são únicos por conta (não globalmente): duas contas diferentes
 * podem ter, cada uma, seu próprio usuário "admin". Isso significa que o mesmo
 * `usuario_login` sozinho não identifica mais um usuário — sempre usar `(conta_id, login)`
 * junto (ver `adm.grupos_permissoes_usuarios`, que carrega `conta_id` redundante com FK
 * composta justamente para impedir vincular um usuário a um grupo de outra conta).
 */
create table adm.usuarios (
  ...
  constraint usuarios_pkey primary key (conta_id, login),
  constraint usuarios_email_ukey unique (conta_id, email),
  constraint usuarios_cpf_ukey unique (conta_id, cpf),
 ...
 *
 * Regras de negócio sobre usuários: login e conta imutáveis, bloqueia criação de logins
 * "sgitec*", exige que quem edita um usuário tenha acesso de administrador (módulo 1) na
 * mesma conta do usuário editado, invalida verificação de e-mail ao trocar e-mail, e
 * impede exclusão de usuário com grupos de permissão vinculados. (Antes da introdução de
 * `adm.usuarios.conta_id`, essa checagem precisava varrer todas as contas às quais um
 * login tinha acesso — desnecessário agora que cada usuário pertence a exatamente uma
 * conta.)
 */
...
begin
  ...
  if new.conta_id <> old.conta_id then
    raise exception 'Não é permitido alterar a conta do usuário.';
  end if;
  ...
  if current_user = 'sgisis' and new.login <> new.updated_by then
    if not exists(
      ...
      where gpu.conta_id = new.conta_id
        and gpu.usuario_login = new.updated_by
        and gp.modulo_id = 1
    ) then
      raise exception 'O usuário % não tem acesso de administrador à conta do usuário %.', new.updated_by, new.login;
    end if;
  end if;
  ...
  if new.deleted_by is not null then
    if exists(select * from adm.grupos_permissoes_usuarios where conta_id = new.conta_id and usuario_login = new.login) then
 ...
 * os parâmetros de identificação vão dentro de `parametros` (jsonb). Como `login` deixou
 * de ser único globalmente (agora é único por conta), identificar o usuário-alvo exige
 * gravar `conta_id` + `login` juntos em `parametros`, não mais só `login`.
 */
 ...
 * grupo "Administrador" com acesso total, vinculado ao usuário "admin" da própria conta
 * (se existir) e a todo usuário já administrador da conta no módulo 1. Demais módulos
 * recebem os grupos "Administrador" e "Consultas e Relatórios".
 */
    ...
    if exists(select * from adm.modulos where id = p_modulo) then
      ...
      if p_modulo = 1 then
        if exists(select * from adm.contas_modulos where conta_id = p_conta and modulo_id = p_modulo) then
          ...
          v_usuario = 'admin';
          if exists(select * from adm.usuarios where conta_id = p_conta and login = v_usuario) then
            if not exists(select * from adm.grupos_permissoes_usuarios where conta_id = p_conta and usuario_login = v_usuario and grupo_permissao_id = (select id from adm.grupos_permissoes where conta_id = p_conta and descricao = v_grupo and modulo_id = p_modulo)) then
              insert into adm.grupos_permissoes_usuarios (conta_id, grupo_permissao_id, usuario_login)
              values (p_conta, (select id from adm.grupos_permissoes where conta_id = p_conta and descricao = v_grupo and modulo_id = p_modulo), v_usuario);
            end if;
          end if;
...
create table adm.grupos_permissoes (
  ...
  constraint grupos_permissoes_conta_id_id_ukey unique (conta_id, id),
 ...
 *
 * `conta_id` é redundante com o que já se pode derivar de `grupo_permissao_id` (via
 * `adm.grupos_permissoes.conta_id`) e de `usuario_login` (via `adm.usuarios.conta_id`) —
 * mas precisa estar aqui como coluna própria para sustentar as duas FKs compostas abaixo,
 * que impedem a nível de banco vincular um usuário a um grupo de permissão de OUTRA conta.
 */
create table adm.grupos_permissoes_usuarios (
  ...
  constraint grupos_permissoes_usuarios_conta_id_grupo_fkey foreign key (conta_id, grupo_permissao_id) references adm.grupos_permissoes (conta_id, id) on delete cascade,
  constraint grupos_permissoes_usuarios_conta_id_login_fkey foreign key (conta_id, usuario_login) references adm.usuarios (conta_id, login) on delete cascade
);
...
create or replace view adm.view_usuarios_contas as
 select c.id,
  ...
  group by c.id, c.razao_social, gp.modulo_id, gpu.usuario_login;
...
create or replace view adm.view_usuarios_consultas as
 select distinct u.login as usuario_login,
    ...
    c.titulo
   from adm.usuarios u
     join adm.grupos_permissoes_usuarios gpu on (gpu.usuario_login = u.login and gpu.conta_id = u.conta_id)
     join adm.grupos_permissoes gp on (gp.id = gpu.grupo_permissao_id)
 ...
 * escrito diretamente pela aplicação (sem trigger de auditoria genérica). `conta_id` é
 * nullable e sem FK (mesmo padrão das demais colunas desta tabela) porque nem toda rota
 * logada tem uma conta resolvida no momento do acesso (ex.: tela de login antes da
 * autenticação) — e porque `usuario_login` sozinho não identifica mais um usuário único
 * agora que o login é escopado por conta.
 */
